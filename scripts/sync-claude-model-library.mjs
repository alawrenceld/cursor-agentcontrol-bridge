#!/usr/bin/env node
// Sync Anthropic catalog prices into LD model library and attach to
// claude-code-usage variations. Adds agentModel targeting rules.
...
import { PRICING_PER_M } from '../src/lib/claudePricing.mjs';
...
const byId = new Map(
  entries.filter((m) => m.provider === 'Anthropic').map((m) => [m.id, m]),
);

const configRes = await api('GET', `/${config.aiConfigKey}`);
if (!configRes.ok) {
  console.error(
    `AI Config "${config.aiConfigKey}" not found — run npm run setup:claude-ai-config first`,
  );
  process.exit(1);
}
...
for (const variationKey of variationKeys) {
  ...
  if (pricing === undefined) {
    console.error(`! ${variationKey}: not in src/lib/claudePricing.mjs — skipped`);
    failures += 1;
    continue;
  }
  if (pricing === null) {
  ...
  const desiredKey = `Anthropic.${catalogId}`;
...
if (instructions.length > 0) {
  const patched = await api('PATCH', `/${config.aiConfigKey}/targeting`, {
    ...
    comment: 'sync-claude-models: agentModel → variation',
    instructions,
  });
  if (!patched.ok) {
