#!/usr/bin/env node
// Bundled by scripts/build-hook.mjs — edit src/judge/judge-worker.mjs instead.
...
function parseTranscript(jsonl) {
  ...
  for (const line of String(jsonl).split("\n")) {
    ...
    try {
      ...
      const content = typeof entry.message === "string" ? entry.message : JSON.stringify(entry.message);
...
function buildJudgeInput({ messages, diff, maxChars = 6e4, maxDiffChars = 2e4 }) {
...
function gitDiff(workspaceRoot) {
  ...
  try {
    const result = (0, import_node_child_process.spawnSync)("git", ["-C", workspaceRoot, "diff", "HEAD"], {
      timeout: 1e4,
  ...
  try {
    ...
    return {
      ...
      tokens: usage ? {
        ...
        total: (usage.inputTokens ?? 0) + (usage.outputTokens ?? 0) + (usage.cacheReadTokens ?? 0) + (usage.cacheWriteTokens ?? 0)
      } : null
    };
  } catch {
...
async function main() {
  ...
  const rubric = (judgeValue.messages ?? []).map((m) => m.content).join("\n").trim();
  ...
  const input = buildJudgeInput({
    messages: transcript,
    diff,
    maxChars: judgeCfg.maxInputChars ?? 6e4
  });
...
${output.slice(0, 1e4)}`;
  ...
  const timeoutMs = judgeCfg.timeoutMs ?? 24e4;
  ...
  try {
    ...
    tracker.track(EVENT_KEYS2.durationTotal, context, judgeTrackData, Date.now() - started);
    tracker.track(EVENT_KEYS2.generationError, context, judgeTrackData, 1);
    await tracker.close();
  ...
  tracker.track(EVENT_KEYS2.durationTotal, context, judgeTrackData, Date.now() - started);
  tracker.track(
    verdict ? EVENT_KEYS2.generationSuccess : EVENT_KEYS2.generationError,
    context,
    judgeTrackData,
    1
  );
  if (judgeTokens?.total > 0) {
    tracker.track(EVENT_KEYS2.tokensTotal, context, judgeTrackData, judgeTokens.total);
    tracker.track(EVENT_KEYS2.tokensInput, context, judgeTrackData, judgeTokens.input);
    tracker.track(EVENT_KEYS2.tokensOutput, context, judgeTrackData, judgeTokens.output);
  }
  if (verdict) {
    tracker.track(
      metricKey,
...
`
    ...
    logLine(
      `scored conv=${job.conversationId} variation=${job.parentTrackData.variationKey} score=${verdict.score} (${Math.round((Date.now() - started) / 1e3)}s): ${verdict.reasoning.slice(0, 200)}`
    );
  } else {
