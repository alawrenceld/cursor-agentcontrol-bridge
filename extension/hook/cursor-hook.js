  ...
  if (!cachedRuntime) {
    try {
      ...
      cachedRuntime = {
        ...
        error: err,
        config: null,
        stateDir: import_node_path3.default.join(PROJECT_ROOT, ".state"),
        sdkKey: void 0
      };
    }
  }
...
var PENDING_DIR = () => import_node_path3.default.join(runtime().stateDir, "pending");
var LOG_FILE = () => import_node_path3.default.join(runtime().stateDir, "hook.log");
function logLine(message) {
  try {
    (0, import_node_fs3.mkdirSync)(import_node_path3.default.dirname(LOG_FILE()), { recursive: true });
    (0, import_node_fs3.appendFileSync)(LOG_FILE(), `${(/* @__PURE__ */ new Date()).toISOString()} ${message}
`);
  } catch {
  }
}
...
function pendingPath(convId) {
  return import_node_path3.default.join(PENDING_DIR(), `${String(convId).replace(/[^A-Za-z0-9_-]/g, "_")}.json`);
}
function handleBeforeSubmitPrompt(payload) {
  ...
  if (convId) {
    ...
    (0, import_node_fs3.writeFileSync)(pendingPath(convId), JSON.stringify({ ts: Date.now() }));
  } else {
...
function takePendingTs(convId) {
  ...
  try {
    const { ts } = JSON.parse((0, import_node_fs3.readFileSync)(file, "utf8"));
...
function cleanupStalePending() {
  try {
    ...
    for (const name of (0, import_node_fs3.readdirSync)(PENDING_DIR())) {
      const file = import_node_path3.default.join(PENDING_DIR(), name);
      if ((0, import_node_fs3.statSync)(file).mtimeMs < cutoff) (0, import_node_fs3.unlinkSync)(file);
    }
  } catch {
  }
}
...
async function handleStop(payload) {
  ...
  try {
    ...
    (0, import_node_fs3.writeFileSync)(
      import_node_path3.default.join(runtime().stateDir, "last-stop-payload.json"),
...
function maybeSpawnJudge(config, payload, trackData, contextKey, dryRun) {
  ...
  try {
    const jobDir = import_node_path3.default.join(runtime().stateDir, "judge-jobs");
    (0, import_node_fs3.mkdirSync)(jobDir, { recursive: true });
    const jobPath = import_node_path3.default.join(jobDir, `${trackData.runId}.json`);
    (0, import_node_fs3.writeFileSync)(
      jobPath,
      JSON.stringify({
    ...
    const sibling = import_node_path3.default.join(import_node_path3.default.dirname(process.argv[1] ?? ""), "judge-worker.js");
    const workerPath = (0, import_node_fs3.existsSync)(sibling) ? sibling : import_node_path3.default.join(PROJECT_ROOT, "src", "judge", "judge-worker.mjs");
